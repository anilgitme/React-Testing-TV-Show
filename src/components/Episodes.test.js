...
const apiData = [
  ...
  test('list episodes correctly', () => {
      const { rerender, getAllByText, queryAllByText } = render(<Episodes episodes={[]}/>)

      const episodesOption = queryAllByText(/episode/i)
      expect(episodesOption).toHaveLength(0)

      rerender(<Episodes episodes={apiData}/>);
      const eData = getAllByText(/episode/i)
      expect(eData).toHaveLength(3)
  })